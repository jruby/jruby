...
public class RubiniusByteArray extends RubyObject {
    ...
    public static void createByteArrayClass(Ruby runtime) {
        RubyClass baClass = runtime
                .getOrCreateModule("Rubinius")
                .defineClassUnder("ByteArray", runtime.getObject(), ObjectAllocator.NOT_ALLOCATABLE_ALLOCATOR);
    ...
    @JRubyMethod(meta = true)
    public static IRubyObject allocate_sized(ThreadContext state, IRubyObject baClass, IRubyObject size) {
        return new RubiniusByteArray(state.runtime, (int)size.convertToInteger().getLongValue());
    }

    @JRubyMethod(name = "new", meta = true)
    public static IRubyObject rbNew(ThreadContext state, IRubyObject baClass, IRubyObject size) {
        return allocate_sized(state, baClass, size);
    }
    ...
    public IRubyObject locate(ThreadContext state, IRubyObject pattern, IRubyObject start, IRubyObject max_o) {
        ...
        for(int i = (int)start.convertToInteger().getLongValue(); i < max; i++) {
          if(this.bytes[i] == pat[0]) {
            ...
            if(j == len) return RubyFixnum.newFixnum(state.runtime, i + len);
          }
        }
    ...
    @JRubyMethod
    public IRubyObject utf8_char(ThreadContext context, IRubyObject offset) {
        // not sure what this is supposed to return or best way to get it in JRuby yet
        throw context.runtime.newNotImplementedError("ByteArray#utf8_char not implemented");
    }
}
