 ...
 * @author  amoore
 */
public class RubyRegexpRej extends RubyObject implements ReOptions {
    ...
    public RubyRegexpRej(Ruby runtime, RubyClass klass) {
        super(runtime, klass);
    }

    private RubyRegexpRej(Ruby runtime) {
    ...
    public IRubyObject op_equal(IRubyObject other) {
        ...
        rb_reg_check(this);
        rb_reg_check((RubyRegexpRej)other);
        if(str.equals(((RubyRegexpRej)other).str) &&
           kcode == ((RubyRegexpRej)other).kcode &&
           ptr.options == ((RubyRegexpRej)other).ptr.options) {
            return getRuntime().getTrue();
        }
        return getRuntime().getFalse();
    }
    ...
    private void rb_reg_check(IRubyObject re) {
        if(((RubyRegexpRej)re).ptr == null || ((RubyRegexpRej)re).str == null) {
    ...
    public IRubyObject initialize_copy(IRubyObject re) {
        ...
        initialize(((RubyRegexpRej)re).str, ((RubyRegexpRej)re).rb_reg_options());
    ...
    public IRubyObject initialize_m(IRubyObject[] args) {
        ...
        if(args[0] instanceof RubyRegexp) {
            ...
            rb_reg_check(args[0]);
            RubyRegexpRej r = (RubyRegexpRej)args[0];
    ...
    public int search(RubyString str, int pos, boolean reverse) {
        ...
        byte[] cstr = bl.bytes;
        result = 1;//ptr.search(cstr,bl.begin,bl.realSize,pos,range,regs);
        ...
        if(match.isNil()) {
            match = new RubyMatchDataRej(getRuntime());
        } else {
        ...
        ((RubyMatchDataRej)match).regs = regs.copy();

        RubyString ss = str.makeShared(0, str.getByteList().realSize);
        ss.freeze();
        ((RubyMatchDataRej)match).str = ss;
    ...
    public IRubyObject match_m(IRubyObject str) {
        ...
        return getRuntime().getCurrentContext().getCurrentFrame().getBackRef();
    }
    ...
    public static RubyRegexpRej regexpValue(IRubyObject obj) {
        if(obj instanceof RubyRegexpRej) {
            return (RubyRegexpRej)obj;
    ...
    public static IRubyObject nth_match(int nth, IRubyObject match) {
        ...
        RubyMatchDataRej m = (RubyMatchDataRej)match;
    ...
    public static IRubyObject last_match_s(IRubyObject recv, IRubyObject[] args) {
        ...
        return recv.getRuntime().getCurrentContext().getCurrentFrame().getBackRef();
    }
    ...
    public static IRubyObject union(IRubyObject recv, IRubyObject[] args) {
        if(args.length == 0) {
            ...
            for(int i = 0; i < args.length; i++) {
                ...
                if(!v.isNil()) {
                    if(!((RubyRegexpRej)v).kcode_default) {
                        if(kcode == null) {
                            kcode_re = v;
                            kcode = ((RubyRegexpRej)v).kcode;
                        } else if(((RubyRegexpRej)v).kcode != kcode) {
    ...
    public static RubyRegexpRej unmarshalFrom(UnmarshalStream input) throws java.io.IOException {
        RubyRegexpRej result = newRegexp(input.getRuntime(), input.unmarshalString(), input.unmarshalInt(), null);
        input.registerLinkTarget(result);
        return result;
    }

    public static void marshalTo(RubyRegexpRej regexp, MarshalStream output) throws java.io.IOException {
    ...
    public static RubyRegexpRej newRegexp(Ruby runtime, String pattern, int options, String kcode) {
        return newRegexp(runtime, ByteList.create(pattern), options, kcode);
    }

    public static RubyRegexpRej newRegexp(IRubyObject ptr, int options, String kcode) {
        return newRegexp(ptr.getRuntime(), ptr.convertToString().getByteList(), options, kcode);
    }

    public static RubyRegexpRej newRegexp(Ruby runtime, ByteList pattern, int options, String kcode) {
        RubyRegexpRej rr = new RubyRegexpRej(runtime);
        rr.initialize(pattern,options);
        return rr;
    }

    @JRubyMethod(name = "new", name2 = "compile", required = 1, optional = 2, meta = true)
    public static RubyRegexpRej newInstance(IRubyObject recv, IRubyObject[] args) {
        RubyClass klass = (RubyClass)recv;
        
        RubyRegexpRej re = (RubyRegexpRej) klass.allocate();
