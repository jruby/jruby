...
def mspec(mspec_options = {})
  ...
  java_cmd << "-T -J-Djruby.compile.mode=#{ms[:compile_mode]}"
  java_cmd << "-T -J-Djruby.jit.threshold=#{ms[:jit_threshold]}"
  java_cmd << "-T -J-Djruby.jit.max=#{ms[:jit_max]}"
  java_cmd << "-T -J-Djruby.objectspace.enabled=#{ms[:objectspace_enabled]}"
  java_cmd << "-T -J-Djruby.thread.pool.enabled=#{ms[:thread_pooling]}"
  java_cmd << "-T -J-Djruby.reflection=#{ms[:reflection]}"
  ...
  java_cmd << "-f #{ms[:format]}"
  java_cmd << "--timeout #{ms[:timeout]}"
  java_cmd << "-B #{ms[:spec_config]}" if ms[:spec_config]
  java_cmd << "#{ms[:spec_target]}" if ms[:spec_target]
