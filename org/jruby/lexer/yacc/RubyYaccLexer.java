...
public class RubyYaccLexer implements IYaccLexer {
	...
	private char readEscape() {
		...
		switch (c = support.read()) {
			...
			case 'M' :
				if ((c = support.read()) != '-') {
					errorHandler.handleError(IErrors.SYNTAX_ERROR, support.getPosition(), Messages.getString("invalid_escape_expected_minus", Character.toString(c))); //$NON-NLS-1$
					support.unread();
					return '\0';
				} else if ((c = support.read()) == '\\') {
					return (char)(readEscape() | 0x80);
				} else if (c == '\0') {
					errorHandler.handleError(IErrors.SYNTAX_ERROR, support.getPosition(), Messages.getString("invalid_escape_unterminated_string")); //$NON-NLS-1$
					return '\0';
				} else {
	...
	private int parseHereDocument(int closeQuote, boolean indent) {
			...
			switch (c = nextc()) {
				...
				case '=' :
					if (support.getPosition().getColumn() == 1) {
						...
						if (support.isNext("begin") && Character.isWhitespace(support.getCharAt(5))) { //$NON-NLS-1$
							for (;;) {
								// col = lex_pend;
								c = nextc();
								if (c == -1) {
									...
									continue;
								} else if (support.getPosition().getColumn() == 1 && support.isNext("end") && Character.isWhitespace(support.getCharAt(3))) { //$NON-NLS-1$
									break;
								}
							}
							support.readLine();
				...
				case '@' :
					...
					if (Character.isDigit((char) c)) {
						errorHandler.handleError(IErrors.COMPILE_ERROR, support.getPosition(), Messages.getString("invalid_instance_variable_name", Character.toString((char)c))); //$NON-NLS-1$
					}
					if (!isIdentifierChar(c)) {
						pushback(c);
						return '@';
				...
				default :
					if (!isIdentifierChar(c) || Character.isDigit((char) c)) {
						errorHandler.handleError(IErrors.COMPILE_ERROR, support.getPosition(), Messages.getString("invalid_char_in_expression", Character.toString((char)c))); //$NON-NLS-1$
						continue retry;
					}
		...
		private int parseQuotation() {
			...
			switch (type) {
				...
				default :
					errorHandler.handleError(IErrors.SYNTAX_ERROR, support.getPosition(), Messages.getString("unknown_quotation_type", Character.toString(type))); //$NON-NLS-1$
					return 0;
			}
		}
