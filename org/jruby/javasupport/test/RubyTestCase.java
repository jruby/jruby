...
public class RubyTestCase extends TestCase {
    ...
    protected Ruby createRuby (String fileName)
        throws IOException
    {
        return createRuby(new File(fileName));
    }

    protected Ruby createRuby (URL url)
        throws IOException
    {
    ...
    protected Ruby createRuby (InputStream in)
        throws IOException
    {
    ...
    protected Ruby createRuby (File file)
        throws IOException
    {
        ...
        RubyKernel.require(ruby, null, new RubyString(ruby, filePath));

        return ruby;
    }

    // Is there something built into JRuby to do this?
    protected void initRuby (Ruby ruby)
        throws IOException
    {
        RubyObject empty = JavaUtil.convertJavaToRuby(ruby, EMPTY_ARRAY,
                                                      EMPTY_ARRAY.getClass());

          ruby.defineReadonlyVariable("$-p", (RubyObject)ruby.getNil());
          ruby.defineReadonlyVariable("$-n", (RubyObject)ruby.getNil());
          ruby.defineReadonlyVariable("$-a", (RubyObject)ruby.getNil());
          ruby.defineReadonlyVariable("$-l", (RubyObject)ruby.getNil());
