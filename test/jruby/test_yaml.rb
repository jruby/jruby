...
EOY
  ...
  def test_JRUBY_1220 # make sure all three variations work
    bad_text = " A\nR"
...
YAML
  ...
  def test_JRUBY_1766
    assert YAML.load(Time.now.to_yaml).instance_of?(Time)
    assert YAML.load("2007-01-01 01:12:34").instance_of?(String)
    assert YAML.load("2007-01-01 01:12:34.0").instance_of?(String)
    assert YAML.load("2007-01-01 01:12:34 +00:00").instance_of?(Time)
    assert YAML.load("2007-01-01 01:12:34.0 +00:00").instance_of?(Time)
    assert YAML.load("{a: 2007-01-01 01:12:34}")["a"].instance_of?(String)
  end
...
YAML
  ...
  def test_JRUBY_1978 # scalars can start with , if it's not ambigous
...
# Make sure that overriding to_yaml always throws an exception unless it returns the correct thing
  ...
  def test_JRUBY_2019
    # handle tagged_classes, yaml_as and so on a bit better
    tagged_classes = YAML::tagged_classes.to_a
    ...
    }.to_a.each {|a| assert tagged_classes.include? a}
  end
  ...
  class ::YAMLTestException < Exception;
  end
  class ::YAMLTestString < String;
  end
...
YAML
  ...
  class ::FooYSmith < Array;
  end

  class ::FooXSmith < Hash;
  end
...
Y
    ...
    assert_nothing_raised { YAML.load(jruby3639) }
  end
...
YAML
  ...
  def test_JRUBY_4345
    ...
    assert_equal ruby_object.to_yaml, YAML.load(ruby_object.to_yaml).to_yaml
  end
