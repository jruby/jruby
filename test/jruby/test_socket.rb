...
class ServerTest < Test::Unit::TestCase
  ...
  # jruby/jruby#8786
  # select(2) reports a non-blocking connect that failed as writable, leaves the error on
  # the socket for getsockopt(SOL_SOCKET, SO_ERROR), and leaves the descriptor valid until
  # the application closes it. Socket.tcp's Happy Eyeballs v2 logic needs all three in
  # order to fall back from one address family to the other.
  def test_select_reports_failed_nonblocking_connect
    ...
    socket = Socket.new(Socket::AF_INET, Socket::SOCK_STREAM, 0)
    result = socket.connect_nonblock(Addrinfo.tcp("127.0.0.1", port), exception: false)

    # on some platforms a refused loopback connect completes right away, leaving
    # nothing to select on
    return unless result == :wait_writable

    writable = nil
    assert_nothing_raised { _, writable, = IO.select(nil, [socket], nil, 10) }
    assert_equal [socket], writable

    assert_equal Errno::ECONNREFUSED::Errno,
                 socket.getsockopt(Socket::SOL_SOCKET, Socket::SO_ERROR).int

    assert_nothing_raised { socket.close }
  ensure
    socket.close rescue nil
    server.close rescue nil
  end
  ...
  # jruby/jruby#8786
  # Socket.tcp with a hostname takes the Happy Eyeballs v2 path, which races an IPv6 and
  # an IPv4 connection attempt. Where "localhost" resolves to both ::1 and 127.0.0.1 but
  # the server listens on 127.0.0.1 only, the IPv6 attempt is refused and the algorithm
  # has to fall back to IPv4 instead of propagating the failure.
  def test_tcp_fast_fallback_when_only_ipv4_is_listening
    server = TCPServer.new("127.0.0.1", 0)
    port = server.addr[1]

    client = Socket.tcp("localhost", port, connect_timeout: 10)
    assert_equal 5, client.write("hello")

    accepted = server.accept
    assert_equal "hello", accepted.read(5)
  ensure
