...
class TestTyper < Test::Unit::TestCase
  ...
  def test_fixnum
    ...
    assert_equal(AST::TypeReference.new(:fixnum), ast.infer_type(Typer::SimpleTyper.new(:bar)))
  end
  
  def test_float
    ast = AST.parse("1.0")
    
    assert_equal(AST::TypeReference.new(:float), ast.infer_type(Typer::SimpleTyper.new(:bar)))
  end
  
  def test_string
    ast = AST.parse("'foo'")
    
    assert_equal(AST::TypeReference.new(:string), ast.infer_type(Typer::SimpleTyper.new(:bar)))
  end
  ...
  def test_body
    ...
    assert_equal(AST::TypeReference.new(:fixnum), ast1.infer_type(Typer::SimpleTyper.new(:bar)))
    assert_equal(nil, ast2.infer_type(Typer::SimpleTyper.new(:bar)))
  end
  
  def test_local
    ast1 = AST.parse("a = 1; a")
    typer = Typer::SimpleTyper.new :bar
  ...
  def test_signature
    ["def foo", "def self.foo"].each do |def_foo|
      ...
      assert_raise(AST::InferenceError) {typer.resolve_deferred(true)}
