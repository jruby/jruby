...
eom
      ...
      def self.linux?(*ver)
        unless defined?(@linux)
          @linux = RbConfig::CONFIG["host_os"].include?('linux') && `uname -r`.scan(/\d+/).map(&:to_i)
        end
      ...
      def self.macos?(*ver)
        unless defined?(@macos)
          @macos = RbConfig::CONFIG["host_os"].include?('darwin') && `sw_vers -productVersion`.scan(/\d+/).map(&:to_i)
        end
