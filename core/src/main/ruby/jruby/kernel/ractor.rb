...
class Ractor
  ...
  def self.new(*args, name: nil, &block)
    ...
    ractor = allocate
    ractor.__send__(:initialize, loc, name, args)
    ractor.__run(&block) if block
    ractor
  end

  attr_reader :name, :loc, :value

  def initialize(loc, name, args)
    @loc = loc
    @name = name == nil ? name : name.respond_to?(:to_str) ? name.to_str : raise(TypeError)
    @args = args
    @store = {}
    @value = nil
    @port = Port.new
  ...
  def inspect
    loc  = self.loc #__builtin_cexpr! %q{ RACTOR_PTR(self)->loc }
    name = self.name #__builtin_cexpr! %q{ RACTOR_PTR(self)->name }
    id   = self.__id__ #__builtin_cexpr! %q{ UINT2NUM(rb_ractor_id(RACTOR_PTR(self))) }
    status = @port.closed? ? :terminated : :running #__builtin_cexpr! %q{
  ...
  def self.store_if_absent(sym)
    return if current.store.key? sym
    current.store[key] = yield
    # Primitive.ractor_local_value_store_if_absent(sym)
  end

  # returns main ractor
  def self.main
    Thread.main[:__ractor__]
    # __builtin_cexpr! %q{
...
Thread.current[:__ractor__] = Ractor.new(name: "main")
