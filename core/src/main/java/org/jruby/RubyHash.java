...
public class RubyHash extends RubyObject implements Map {
    ...
    // TODO: what are we doing with the initialTable here?
    // I think even for RubyHashEntries this does not make much sense as we do not have a HEAD, right?
    protected RubyHash(Ruby runtime, RubyClass metaClass, IRubyObject defaultValue, IRubyObject[] initialTable, int threshold) {
    ...
    private final synchronized void resize(int newCapacity) {
        ...
        final int[] newBins = new int[newCapacity * NUMBER_OF_ENTRIES];
    ...
    private final void MRICheckResize() {
        if (entries.length == (size * NUMBER_OF_ENTRIES)) {
    ...
    private final IRubyObject internalDeleteLinearSearch(final int hash, final EntryMatchType matchType, final IRubyObject key, final IRubyObject value) {
        ...
        for(int i = start; i < end * NUMBER_OF_ENTRIES; i++) {
            otherKey = entries[i];
            otherValue = entries[i + 1];
            if (otherKey == null)
                continue;
            ...
            if (matchType.matches(key, value, otherKey, otherValue)) {
              ...
              if ((i / NUMBER_OF_ENTRIES) == start)
                  start++;
              if ((i / NUMBER_OF_ENTRIES) == end)
                  end--;
              return otherValue;
            }
        }
    ...
    private void rehashLinearSearch() {
        ...
        for(int i = 0; i < end; i++) {
            ...
            if (!exists) {
                newEntries[i * NUMBER_OF_ENTRIES] = key;
                newEntries[(i * NUMBER_OF_ENTRIES) + 1] = entries[(i * NUMBER_OF_ENTRIES) + 1];
                newHashes[i] = newHash;
            }
        }
    ...
    public IRubyObject shift(ThreadContext context) {
        ...
        key = entries[start * NUMBER_OF_ENTRIES];
        lastKey = entries[(end - 1) * NUMBER_OF_ENTRIES];
        value = entries[(start * NUMBER_OF_ENTRIES) + 1];
        if (key != lastKey) {
    ...
    private class BaseIterator implements Iterator {
        ...
        private int startGeneration, index;
