...
//        }
    ...
    public void codegenScope(IRScope scope) {
        if (usesEvalAtSomeLevel(scope)) {
            // Methods that use eval can't be compiled because the IR logic for eval requires that the parent scope
            // be able to provide an IRScope object to be the lexical parent for the newly-compiled eval string. This
            // is not possible to do in compiled code, since we only save the essence of the original IR in the
            // generated bytecode. For now, we disable JIT for methods that contain evals.
    ...
    private boolean usesEvalAtSomeLevel(IRScope scope) {
        if (scope.usesEval()) return true;

        for (IRScope closure : scope.getClosures()) {
            if (usesEvalAtSomeLevel(closure)) return true;
        }
