...
public class RespondToSite extends MutableCallSite {
    ...
    public RespondToSite(MethodType type, String name, String file, int line) {
        super(type);
    ...
    public static final Handle RESPOND_TO_BOOTSTRAP = Bootstrap.getBootstrapHandle("respondToBootstrap", RespondToSite.class, sig(CallSite.class, MethodHandles.Lookup.class, String.class, MethodType.class, String.class, int.class));

    public static CallSite respondToBootstrap(MethodHandles.Lookup lookup, String name, MethodType methodType, String file, int line) {
        RespondToSite respondToSite = new RespondToSite(methodType, JavaNameMangler.demangleMethodName(name), file, line);
    ...
    public IRubyObject respondToFallback(ThreadContext context, IRubyObject self) throws Throwable {
        ...
        if (!InvokeSite.methodMissing(entry.method)) {
            if (entry.method.isBuiltin()) {
                ...
                boolean respondsTo = self.getMetaClass().respondsToMethod(name, false);
                if (respondsTo) {
        ...
        if (target == null) {
            target = Binder.from(type())
                    .append(IRubyObject.class, Convert.asSymbol(context, name))
                    .invoke(SelfInvokeSite.bootstrap(lookup(), name, methodType(IRubyObject.class, ThreadContext.class, IRubyObject.class, IRubyObject.class), 0, 0, file, line).dynamicInvoker());
        }

        target = typeCheck(target, Signature.from(type().returnType(), type().parameterArray(), "context", "self"), self, self.getMetaClass(), getTarget());
        target = InvokeSite.switchPoint(switchPoint, getTarget(), target);
    ...
    public IRubyObject respondToFallback(ThreadContext context, IRubyObject caller, IRubyObject self) throws Throwable {
        ...
        if (!InvokeSite.methodMissing(entry.method, name, CallType.NORMAL, caller)) {
            if (entry.method.isBuiltin()) {
                // standard respond_to, check if method is naturally defined (not via respond_to_missing?)
                boolean respondsTo = self.getMetaClass().respondsToMethod(name, false);
                if (respondsTo) {
        ...
        if (target == null) {
            target = Binder.from(type())
                    .append(IRubyObject.class, Convert.asSymbol(context, name))
                    .invoke(SelfInvokeSite.bootstrap(lookup(), name, type().appendParameterTypes(IRubyObject.class), 0, 0, file, line).dynamicInvoker());
        }
