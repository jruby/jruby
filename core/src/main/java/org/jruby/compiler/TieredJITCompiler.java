...
public class TieredJITCompiler implements JITCompilerMBean, JITCompiler {
    private static final Logger LOG = LoggerFactory.getLogger(TieredJITCompiler.class);
    ...
    public TieredJITCompiler(Ruby runtime) {
    ...
    @Override
    public void queueForJIT(ThreadContext context, final Compilable method) {
        ...
        if (task instanceof Task && config.getJitMax() >= 0 && config.getJitMax() < getSuccessCount()) {
            if (config.isJitLogging()) {
                TieredJITCompiler.log(method, method.getName(), "skipping: jit.max threshold reached");
            }
            return;
        }
    ...
    static abstract class Task implements Runnable {
        ...
        protected final TieredJITCompiler jitCompiler;

        public Task(TieredJITCompiler jitCompiler) {
