...
public class RubyString extends RubyObject implements CharSequence, EncodingCapable, MarshalEncoding, CodeRangeable, Appendable {
    ...
    public IRubyObject initialize(ThreadContext context, IRubyObject arg0, IRubyObject opts) {
        ...
        if (tmp.isNil()) throw context.runtime.newArgumentError(2, 1);
    ...
    private IRubyObject subBangIter(ThreadContext context, IRubyObject arg0, RubyHash hash, Block block) {
        if (arg0 instanceof RubyRegexp regexp) {
            return subBangIter(context, regexp, hash, block);
        } else {
    ...
    public IRubyObject op_aref(ThreadContext context, IRubyObject arg) {
        ...
        if (arg instanceof RubyFixnum fixnum) {
            return op_aref(runtime, asInt(context, fixnum));
            ...
            int[] begLen = range.begLenInt(strLength(), 0);
    ...
    public IRubyObject op_aref(ThreadContext context, IRubyObject arg1, IRubyObject arg2) {
        Ruby runtime = context.runtime;
        if (arg1 instanceof RubyRegexp regexp) return subpat(context, regexp, arg2);
        return substrEnc(runtime, RubyNumeric.num2int(arg1), RubyNumeric.num2int(arg2));
    }
    ...
    public IRubyObject op_aset(ThreadContext context, IRubyObject arg0, IRubyObject arg1) {
        if (arg0 instanceof RubyFixnum fixnum) {
            return op_aset(context, asInt(context, fixnum), arg1);
        } else if (arg0 instanceof RubyRegexp regexp) {
            ...
            int[] begLen = range.begLenInt(strLength(), 2);
            RubyString repl = arg1.convertToString();
    ...
    final IRubyObject uptoCommon(ThreadContext context, RubyString end, boolean excl, Block block, boolean asSymbol) {
        ...
        if (value.getRealSize() == 1 && end.value.getRealSize() == 1 && isAscii) {
            ...
            while (true) {
                ...
                block.yield(context, asSymbol ? newSymbol(context, s) : newStringShared(context.runtime, s, enc, CR_7BIT));
            ...
            RubyArray argsArr = newArray(context, newFixnum(context, value.length()), context.nil);
            ...
            if (b instanceof RubyFixnum && e instanceof RubyFixnum) {
                ...
                while (bl <= el) {
                    if (excl && bl == el) break;
                    argsArr.eltSetOk(1, newFixnum(context, bl));
                    ...
                    block.yield(context, asSymbol ? newSymbol(context, str.toString()) : str);
                    bl++;
                    context.pollThreadEvents();
                }
            } else {
    ...
    public IRubyObject getbyte(ThreadContext context, IRubyObject index) {
        ...
        return newFixnum(context, value.getUnsafeBytes()[value.getBegin() + i] & 0xff);
    }
    ...
    public IRubyObject partition(ThreadContext context, IRubyObject arg, Block block) {
        ...
        if (arg instanceof RubyRegexp regexp) {
            if (regexp.search(context, this, 0, false) < 0) return partitionMismatch(runtime);
        ...
        return newArrayNoCopy(context, makeSharedString(runtime, 0, pos), sep,
    ...
    public IRubyObject rpartition(ThreadContext context, IRubyObject arg) {
        ...
        int len = value.getRealSize();
        return newArrayNoCopy(context, substrEnc(context.runtime, 0, pos), sep, substrEnc(context.runtime, beg, len));
    }
    ...
    public IRubyObject count(ThreadContext context, IRubyObject arg) {
        ...
        return asFixnum(context, StringSupport.strCount(value, runtime, table, tables, enc));
    }
        ...
        switch (getCodeRange()) {
            ...
            case CR_7BIT:
                for (int i = 0; i < len; i += n) {
                    ...
                    IRubyObject substr = str.substr(context.runtime, i, n);
    ...
    private IRubyObject enumerateCodepoints(ThreadContext context, String name, Block block, boolean wantarray) {
        ...
        while (ptr < end) {
            ...
            if (wantarray) ary.append(context, newFixnum(context, c));
    ...
    private IRubyObject enumerateBytes(ThreadContext context, String name, Block block, boolean wantarray) {
        ...
        for (int i=0; i < value.getRealSize(); i++) {
            RubyFixnum bite = newFixnum(context, value.get(i) & 0xFF);
    ...
    private static long unpackOffset(ThreadContext context, IRubyObject opt) {
        ...
        long offset = 0;
        if (options.size() == 1) {
            IRubyObject offsetArg = options.fastARef(newSymbol(context, "offset"));
