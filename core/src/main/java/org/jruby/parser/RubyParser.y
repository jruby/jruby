...
%type <Integer> k_return k_class k_module
...
bodystmt      : compstmt opt_rescue opt_else opt_ensure {
                  ...
                  if ($2 != null) {
                      node = new RescueNode(support.line($1, $2), $1, $2, $3);
                  } else if ($3 != null) {
                      support.getWarnings().warn(ID.ELSE_WITHOUT_RESCUE, lexer.getFile(), support.line($1, $3), "else without rescue is useless");
                      node = support.appendToBlock($1, $3);
                  }
                  if ($4 != null) {
                      if (node != null) {
                          node = new EnsureNode(support.line($1, $4), support.makeNullNil(node), $4);
                      } else {
...
stmt            : keyword_alias fitem {
                ...
                | stmt modifier_if expr_value {
                    $$ = new IfNode(support.line($1), support.getConditionNode($3), $1, null);
                    support.fixpos($<Node>$, $3);
                }
                | stmt modifier_unless expr_value {
                    $$ = new IfNode(support.line($1), support.getConditionNode($3), null, $1);
                    support.fixpos($<Node>$, $3);
                }
                | stmt modifier_while expr_value {
                    if ($1 != null && $1 instanceof BeginNode) {
                        $$ = new WhileNode(support.line($1), support.getConditionNode($3), $<BeginNode>1.getBodyNode(), false);
                    } else {
                        $$ = new WhileNode(support.line($1), support.getConditionNode($3), $1, true);
                    }
                }
                | stmt modifier_until expr_value {
                    if ($1 != null && $1 instanceof BeginNode) {
                        $$ = new UntilNode(support.line($1), support.getConditionNode($3), $<BeginNode>1.getBodyNode(), false);
                    } else {
                        $$ = new UntilNode(support.line($1), support.getConditionNode($3), $1, true);
                    }
                }
                ...
                | keyword_END tLCURLY compstmt tRCURLY {
                    if (support.isInDef()) {
                        support.getWarnings().warn(ID.END_IN_METHOD, lexer.getFile(), $1, "END in method; use at_exit");
                    }
                    $$ = new PostExeNode($1, $3);
                }
                | command_asgn
...
command_asgn    : lhs '=' command_rhs {
  ...
  // FIXME: arg_concat logic missing for opt_call_args
                ...
                | primary_value call_op tIDENTIFIER tOP_ASGN command_rhs {
                    value_expr(lexer, $5);
                    $$ = support.newOpAsgn(support.line($1), $1, $2, $5, $3, $4);
                }
...
expr            : command_call
                | expr keyword_and expr {
                    $$ = support.newAndNode(support.line($1), $1, $3);
                }
                | expr keyword_or expr {
                    $$ = support.newOrNode(support.line($1), $1, $3);
                }
...
cpath           : tCOLON3 cname {
                ...
                | primary_value tCOLON2 cname {
                    $$ = support.new_colon2(support.line($1), $1, $3);
                }
...
arg             : lhs '=' arg_rhs {
  ...
  // FIXME: arg_concat missing for opt_call_args
                ...
                | arg tDOT2 arg {
                    ...
                    $$ = new DotNode(support.line($1), support.makeNullNil($1), support.makeNullNil($3), false, isLiteral);
                }
                | arg tDOT3 arg {
                    value_expr(lexer, $1);
                    value_expr(lexer, $3);
                    ...
                    boolean isLiteral = $1 instanceof FixnumNode && $3 instanceof FixnumNode;
                    $$ = new DotNode(support.line($1), support.makeNullNil($1), support.makeNullNil($3), true, isLiteral);
                }
                | arg tPLUS arg {
                ...
                | arg tANDOP arg {
                    $$ = support.newAndNode($1.getLine(), $1, $3);
                }
                | arg tOROP arg {
                    $$ = support.newOrNode($1.getLine(), $1, $3);
                }
                ...
                | arg '?' arg opt_nl ':' arg {
                    value_expr(lexer, $1);
                    $$ = new IfNode(support.line($1), support.getConditionNode($1), $3, $6);
                }
                | primary {
                    $$ = $1;
                }
...
rel_expr        : arg relop arg   %prec tGT {
		...
		| rel_expr relop arg   %prec tGT {
                     support.getWarnings().warning(ID.MISCELLANEOUS, lexer.getFile(), lexer.getRubySourceline(), "comparison '" + $2 + "' after comparison");
                     $$ = support.getOperatorCallNode($1, $2, $3, lexer.getRubySourceline());
                }
...
call_args       : command {
                    value_expr(lexer, $1);
                    $$ = support.newArrayNode(support.line($1), $1);
                }
                | args opt_block_arg {
...
block_arg       : tAMPER arg_value {
                    $$ = new BlockPassNode(support.line($2), $2);
                }
...
args            : arg_value { // ArrayNode
                    int pos = $1 == null ? lexer.getRubySourceline() : $1.getLine();
                    $$ = support.newArrayNode(pos, $1);
                }
                | tSTAR arg_value { // SplatNode
                    $$ = support.newSplatNode(support.line($2), $2);
                }
                ...
                | args ',' tSTAR arg_value { // ArgsCatNode, SplatNode, ArrayNode
                    ...
                    if ($4 instanceof ArrayNode &&
                        ...
                        $$ = support.arg_concat(support.line($1), $1, $4);
                    }
                }
...
mrhs            : args ',' arg_value {
                ...
                | args ',' tSTAR arg_value {
                    ...
                    if ($4 instanceof ArrayNode &&
                        ...
                        $$ = support.arg_concat($1.getLine(), $1, $4);
                    }
                }
                | tSTAR arg_value {
                     $$ = support.newSplatNode(support.line($2), $2);
...
f_margs         : f_marg_list {
                ...
                | tSTAR ',' f_marg_list {
                    $$ = new MultipleAsgnNode(support.line($3), null, null, $3);
                }
...
block_param     : f_arg ',' f_block_optarg ',' f_rest_arg opt_block_args_tail {
                ...
                | f_block_optarg ',' f_rest_arg opt_block_args_tail {
                    $$ = support.new_args(support.line($1), null, $1, $3, null, $4);
                }
                | f_block_optarg ',' f_rest_arg ',' f_arg opt_block_args_tail {
                    $$ = support.new_args(support.line($1), null, $1, $3, $5, $6);
                }
                | f_block_optarg opt_block_args_tail {
                    $$ = support.new_args(support.line($1), null, $1, null, null, $2);
                }
...
lambda          : /* none */  {
                    ...
                    $$ = new LambdaNode($2.getLine(), $2, $4, support.getCurrentScope());
...
brace_body      : {
                    ...
                    lexer.getCmdArgumentState().reset();
                     ...
                     $$ = new IterNode($<Integer>1, $3, $4, support.getCurrentScope());
                     support.popCurrentScope();
                    lexer.getCmdArgumentState().reset($<Long>2.longValue());
                }
...
regexp          : tREGEXP_BEG regexp_contents tREGEXP_END {
                    $$ = support.newRegexpNode(support.line($2), $2, (RegexpNode) $3);
                }
...
string_content  : tSTRING_CONTENT {
                ...
                | tSTRING_DVAR {
                    ...
                    $$ = new EvStrNode(support.line($3), $3);
                }
                | tSTRING_DBEG {
                   $$ = lexer.getStrTerm();
                   lexer.setStrTerm(null);
                   ...
                   $$ = support.newEvStrNode(support.line($7), $7);
                }
...
f_kw            : f_label arg_value {
                ...
                | f_label {
                    lexer.setCurrentArg(null);
                    $$ = support.keyword_arg(lexer.getRubySourceline(), support.assignableKeyword($1, new RequiredKeywordArgumentValueNode()));
                }

f_block_kw      : f_label primary_value {
                    $$ = support.keyword_arg(support.line($2), support.assignableKeyword($1, $2));
                }
                | f_label {
                    $$ = support.keyword_arg(lexer.getRubySourceline(), support.assignableKeyword($1, new RequiredKeywordArgumentValueNode()));
                }
...
f_opt           : f_arg_asgn '=' arg_value {
                    lexer.setCurrentArg(null);
                    $$ = new OptArgNode(support.line($3), support.assignableLabelOrIdentifier($1.getName().getBytes(), $3));
                }
...
f_rest_arg      : restarg_mark tIDENTIFIER {
  ...
  $$ = new UnnamedRestArgNode(lexer.getRubySourceline(), support.symbolID(new ByteList(new byte[] {})), support.getCurrentScope().addVariable("*"));
                }
...
assoc           : arg_value tASSOC arg_value {
                ...
                | tLABEL arg_value {
                    Node label = support.asSymbol(support.line($2), $1);
                    $$ = support.createKeyValue(label, $2);
                }
                | tSTRING_BEG string_contents tLABEL_END arg_value {
                    if ($2 instanceof StrNode) {
                        DStrNode dnode = new DStrNode(support.line($2), lexer.getEncoding());
                        dnode.add($2);
                        $$ = support.createKeyValue(new DSymbolNode(support.line($2), dnode), $4);
                    } else if ($2 instanceof DStrNode) {
                        $$ = support.createKeyValue(new DSymbolNode(support.line($2), $<DStrNode>2), $4);
                    } else {
